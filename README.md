...
- `TrmWrite-Setup-2.1.4.exe`：安装版。
- `TrmWrite-2.1.4-x64.zip`：免安装压缩版，解压后运行。
- `TrmWrite-Portable-2.1.4-x64.exe`：单文件便携版，无需安装和解压。

推送形如 `v2.1.4` 的标签后，GitHub 工作流会自动构建并把三种文件上传到对应版本页面，并附带 SHA-256 校验文件。
